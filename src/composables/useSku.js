...
import {
    createGoodsSkusCard, updateGoodsSkusCard, deleteGoodsSkusCard, sortGoodsSkusCard,
    createGoodsSkusCardValue, updateGoodsSkusCardValue, deleteGoodsSkusCardValue, chooseAndSetGoodsCard
} from '@/api/goods.js'
import { toast, useArrayMoveUp, useArrayMoveDown, cartesianProductOf } from '@/composables/util'
...
function getTableData() {
    setTimeout(() => {
        ...
        // 获取之前的规格列表，将里面的规格ID排序后转为字符串
        let beforeSkuList = JSON.parse(JSON.stringify(sku_list.value)).map(o=>{
            if(!Array.isArray(o.skus)){
                o.skus = Object.keys(o.skus).map(k=>o.skus[k]) // 如果是对象，转为数组
            }
            o.skusId = o.skus.sort((a,b) => a.id-b.id).map(s=>s.id).join(',') //eg:'298,282'
            return o
        })
