...
// 审核商品
async function handleCheckGoods(id, ischeck) {
  const res = await checkGoods(id, ischeck);
  if (res) {
    ischeck === 1 ? toast("审核通过") : toast("审核拒绝");
  }
}
</script>
